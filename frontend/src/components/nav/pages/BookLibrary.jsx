...
export default function BookLibrary(props) {
    ...
    async function handleDelete() {
        const resp = await fetch(`http://localhost:53706/api/media?type=Books&id=${activeEntry.id}`, {
            method: "DELETE"
        })
        if (resp.ok) {
    ...
    return <>
        <Container>
        {
            ...
            <h1 className="no-entries-message">No books to display!</h1>
        }
        </Container>
        ...
        <AddModal show={showAddModal} hide={handleHideAddModal} confirm={handleAdd} type="Books"/>

        <EditModal show={showEditModal} hide={handleHideEditModal} confirm={handleEdit} type="Books"/>
